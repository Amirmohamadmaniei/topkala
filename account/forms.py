...
class OTPForm(forms.Form):
    code = forms.CharField(
        widget=forms.TextInput(attrs={'class': 'input-field', 'placeholder': 'کد ارسال شده را وارد کنید'}))
...
class ForgetPasswordForm(forms.Form):
    ...
    def clean_phone(self):
        ...
        user = User.objects.filter(phone=phone).exists()
        if not user:
            raise ValidationError('شماره تلفن قبلا ثبت نشده است , لطفا ابتدا ثبت نام کنید')
        return phone
